...
import { BRAND, NAV_LINKS, NAV_CTA } from "../data/siteContent";
...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      ...
      { yPercent: 0, opacity: 1, duration: 1.1, ease: "power3.out", delay: 0.4 },
    );
  }, []);
      ...
      <header ref={navRef} className={`navbar ${scrolled ? "navbar--solid" : ""}`}>
        <div className="navbar__inner container">
          <a href="#top" className="navbar__brand" onClick={(e) => handleNavClick(e, "#top")}>
            {BRAND.wordmark}
            <span className="navbar__brand-sub">{BRAND.tagline}</span>
          </a>
