...
export function MobileMenu({ open, onClose, onNavigate }: MobileMenuProps) {
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      gsap
        .timeline()
        .fromTo(panel, { clipPath: "inset(0 0 100% 0)" }, { clipPath: "inset(0 0 0% 0)", duration: 0.6, ease: "power4.inOut" })
        .fromTo(links, { yPercent: 100, opacity: 0 }, { yPercent: 0, opacity: 1, duration: 0.7, ease: "power3.out", stagger: 0.06 }, "-=0.25");
